...
public class App {
	...
	public static void main(String[] args){
		...
		//System.out.println(resultCountMap);
	}
	...
	private static Map<String, Integer> addWordsFromUrl(String url) {//, Map<String, Integer> wordCountMap
		Document doc;
		try {
			doc = Jsoup.connect(url).get();
			String text = doc.body().text();
			//System.out.println("Adding Words From Url : "+url);
			//Creating BufferedReader so the words can be counted
	...
	public static void recursiveCrawlFromUrl(String url, int recurDepth) throws IOException {
		//System.out.println("\nRecur depth : " + recurDepth + " & Url : "+url);
		if(recurDepth > 4) {
			return;
		}
		try {
